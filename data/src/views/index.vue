...
<template>
  <div :class="[isPc?'bg':'bga']" v-if="key.a">
    ...
    <audio ref="audio" src="~@/assets/mp3/xibao.mp3"></audio>
    ...
    <van-dialog v-model="showa" title="标题" show-cancel-button @confirm="start" @cancel="start">
      <van-field v-model="tel" label="验证手机号" placeholder="请输入验证手机号" />
      <van-field v-model="yzm" center clearable label="短信验证码" placeholder="输入短信验证码">
        <template #button>
          <van-button size="small" type="primary" @click.stop="getCode" :disabled="disabled">{{  indexTime}}发送验证码</van-button>
        </template>
      </van-field>
    </van-dialog>
  </div>
...
axios.defaults.baseURL = "http://admin.guangyizhou.cn:8083";
export default {
  name: "home",
  data() {
    return {
  ...
  methods: {
    getCode() {
      if (this.tel) {
        ...
        this.$toast.fail("无查看权限");
      }
    },
    start() {
      if (this.tel && this.yzm) {
        this.disabled = false;
        axios("/client/Login/verifications", {
          method: "post",
    ...
    init() {
      let ws = "ws://admin.guangyizhou.cn:3000";
      let socket = IO(ws);
