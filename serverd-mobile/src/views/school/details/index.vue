...
<template>
  <div class="box">
    <div class="video">
      <video ref="video" :src="play.url" preload="load" controls="controls" playsinline="true" webkit-playsinline="true">
        <source :src="play.url" />
      </video>
    </div>
            ...
            <van-collapse-item :title="item.title" :name="''+index" :key="index">
              <van-list>
                <van-cell v-for="(k,i) in item.video" :key="i">
                  <div class="video-item" :class="{active:k.id===play.id}" @click.stop="toPlay(k,item)">{{k.title}}</div>
                </van-cell>
              </van-list>
            </van-collapse-item>
          </template>
...
export default {
  ...
  methods: {
    async toPlay(k) {
      ...
      video.ontimeupdate = () => {
        if (video.currentTime < time + 20) return;
        time = video.currentTime;
        this.setTime(video.currentTime);
      };
      video.onpause = () => {
      ...
      video.onended = () => {
        this.setTime(video.currentTime, 1);
      };
      await this.axios('/studyapi/video/times', {
        params: { id: this.play.id }
      });
    },
    async setTime(time, status = 0) {
      await this.axios('/studyapi/video/time', {
        params: {
    ...
    async getData() {
      ...
      if (data.code) {
        ...
        this.list = data.data.data && data.data.data.reverse();
