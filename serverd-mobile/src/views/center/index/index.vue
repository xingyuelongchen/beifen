  ...
  <div class="box">
    ...
    <div class="list">
      <van-list>
        ...
        <van-cell class="item" @click.native="to('pay')">
          <div>
    ...
    <van-popup v-model="cover" :close-on-click-overlay="false" closeable>
      <div class="cover">
        <div>
          <vueCropper ref="cropper" :img="img.base64" :autoCrop="true" :info="false" :canScale="true" :fixedBox="true" :fixed="true" :fixedNumber="[1,1]" :canMoveBox="false" :centerBox="true" :infoTrue="true" :full="true" :high="false" mode="100% auto" />
        </div>
        <!-- <cover :imgUrl="img.base64" /> -->
      </div>
      <van-button type="primary" block @click="onUpdate">确定</van-button>
    </van-popup>
    <van-dialog v-model="showUserName" title="昵称修改" show-cancel-button @confirm="changeUserName" @cancel="showUserName = false">
...
export default {
  ...
  methods: {
    async onSelect(item) {
      ...
      if (item.type == 'changinfo') {
        this.$router.push('changinfo');
      }
    },
    ...
    changeUser(type, value) {
      ...
      this.setcookie('userInfo', userInfo);
    ...
    to(val) {
      if (val == 'sign') {
        window.location.href = 'http://com.guangyizhou.cn/sign?redirect_url=http://sm.guangyizhou.cn/#/center/index';
        //  this.$router.push('/sign');
      }else if(val == 'contract'){
         window.location.href = 'http://com.guangyizhou.cn/sign/see?redirect_url=http://sm.guangyizhou.cn/#/center/index';
      } else {
