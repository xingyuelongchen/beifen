...
<template>
  <div class="contract">
    <van-sticky>
      <van-nav-bar title="合同预览" left-text="返回" left-arrow @click-left="back" />
    </van-sticky>
    ...
    <div class="bottom">
      <div class="btn">
        ...
        <van-button type="danger" v-if="!id" @click="back">返回</van-button>
...
export default {
  ...
  methods: {
    async getViews() {
      ...
      let { data } = await this.axios('http://admin.guangyizhou.cn/client/Meal/see_contract', {
    ...
    async getData() {
      let { data } = await this.axios('http://admin.guangyizhou.cn/client/Meal/list')
      if (data.code) {
    ...
    back() {
      let url = this.getstore('redirect_url')
      this.removestore('redirect_url')
      setTimeout(() => {
